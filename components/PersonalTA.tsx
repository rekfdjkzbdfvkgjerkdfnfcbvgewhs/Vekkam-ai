...
import { Send, Loader2, Sparkles, ThumbsUp, ThumbsDown, Check, GraduationCap, Link, Trash2 } from 'lucide-react';
...
const PersonalTA: React.FC<PersonalTAProps> = ({ sessions, studyGroups = [], userBadges = [] }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const { sources } = await queryStrategyTA(
        userMsg, 
        history, 
        sessions, 
        studyGroups, 
  ...
  const suggestionChips = [
    ...
    "Explain the hardest concept I missed",
      ...
      <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-950 z-10 shadow-sm">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-1">
             Active Context: {sessions.length} units
          </p>
        </div>
        {messages.length > 0 && (
          <button 
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-8 max-w-md mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Ready to learn?</h3>
              <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                I'm connected to your syllabus. Ask me anything.
              </p>
            </div>
                ...
                <div className={`p-5 rounded-2xl shadow-sm leading-relaxed text-[15px] ${
                  m.role === 'user' 
                   ...
                   {m.role === 'user' ? (
                     ...
                     <div className="prose prose-sm prose-blue dark:prose-invert max-w-none prose-p:leading-relaxed prose-headings:font-bold prose-a:text-blue-600">
                       {m.content === "" && loading ? (
